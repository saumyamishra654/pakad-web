    ...
    <div className="border border-border rounded-xl overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {songs.map((song) => {
            const row = (
              <tr key={song.id} className="border-b border-border last:border-b-0 hover:bg-bg-card/50 transition-colors">
            ...
            if (song.status === "complete") return <Link key={song.id} href={`/song/${song.id}`} className="contents">{row}</Link>;
            return row;
          })}
        </tbody>
      </table>
